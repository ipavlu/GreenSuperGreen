...
namespace GreenSuperGreen.Benchmarking
{
	...
	public static class BenchTest
	{
		...
		{
			new BenchInfo() {KeyName = "c", CollectorSupport = false, Name =  nameof(DataCollecor), Task = (bi,pc,t,a) => DataCollecor(pc)},
		...
		private static async Task TestsExecute(this IDictionary<string, BenchInfo> testsInfo, IPerfCounterCollectorUC perfCollector)
		{
			while (true)
			{
				...
				int spin;
				if (!int.TryParse(spinText, out spin) || spin < 0) continue;
	...
	public class SyncPrimitivesBenchmark
	{
		...
		public async Task NeighborAsyncLockUC(BenchInfo info, IPerfCounterCollectorUC perfColletor, TimeSpan ts, int spin, int threadPairs)
		{
			await GeneralBenchmark(perfColletor, info.Name, threadPairs, i => new AsyncLockUC(), (i, perfCntr, sync) => new BenchInstance[] { new AsyncLockWorker(perfCntr, sync, ts, spin, $"A{i}"), new AsyncLockWorker(perfCntr, sync, ts, spin, $"B{i}") });
		}

		public async Task NeighborLockUC(BenchInfo info, IPerfCounterCollectorUC perfColletor, TimeSpan ts, int spin, int threadPairs)
		{
			await GeneralBenchmark(perfColletor, info.Name, threadPairs, i => new LockUC(), (i, perfCntr, sync) => new BenchInstance[] { new SimpleLockWorker(perfCntr, sync, ts, spin, $"A{i}"), new SimpleLockWorker(perfCntr, sync, ts, spin, $"B{i}") });
		}

		public async Task NeighborSpinLockUC(BenchInfo info, IPerfCounterCollectorUC perfColletor, TimeSpan ts, int spin, int threadPairs)
		{
			await GeneralBenchmark(perfColletor, info.Name, threadPairs, i => new SpinLockUC(), (i, perfCntr, sync) => new BenchInstance[] { new SimpleLockWorker(perfCntr, sync, ts, spin, $"A{i}"), new SimpleLockWorker(perfCntr, sync, ts, spin, $"B{i}") });
		}
